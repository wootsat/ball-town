...
(function () {
  ...
  // City-less teams (e.g. English Premier League) — addable via the search
  // "+" but not tied to any metro, so they only live in the registry.
  (window.BALLTOWN.extraTeams || []).forEach((t) => {
    const id = idOf(t);
    if (!REGISTRY[id]) REGISTRY[id] = { team: t, cityName: t.city || "" };
  });
