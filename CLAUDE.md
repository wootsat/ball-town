...
- `index.html` — city picker. Cards between the markers are generated by
  build.mjs, **sorted alphabetically by city name**, each with a
  `data-search` attr (city + every team name/abbr). A small inline script
  does the live filter; the search input + empty-state are hand-authored
  in the file (outside the markers, so rebuilds preserve them).
